...
class BunnySync(BaseSync):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                logger.error(f"Failed to connect to Bunny.net: {response.status_code} {response.text}")
                return False
    ...
    def test_connection(self) -> Dict[str, bool]:
        ...
        results = {
            'api_key': False,
            'password': False if self.password else None,
        ...
        if self.password and self.authenticated_cdn_endpoint:
            try:
                ...
                if response.status_code in (200, 404):  # 404 acceptable if directory empty
    ...
    def list_remote_files(self, path: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        ...
        if self.bunny_client:
            try:
                ...
                try:
                    ...
                    logger.error(f"Error with bunnycdnpython GetStoragedObjectsList for path '{path}': {str(e)}")
    ...
    def download_file(self, remote_path: str, local_file_path: str) -> bool:
        ...
        if use_password and not self.authenticated_cdn_endpoint:
            logger.error("Password authentication requested but no authenticated_cdn_endpoint configured")
            return False

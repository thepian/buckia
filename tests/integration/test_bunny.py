...
def test_bunny_file_operations(buckia_client, test_file_factory, remote_test_prefix, cleanup_remote_files):
...
def test_bunny_sync_upload_only(buckia_client, test_directory_factory, remote_test_prefix, cleanup_remote_files):
...
@pytest.mark.skipif(not os.environ.get("RUN_PURGE_TESTS"), reason="Purge tests are slow and require pull zone configuration")
def test_bunny_purge_cache(buckia_client, test_file_factory, remote_test_prefix, cleanup_remote_files):

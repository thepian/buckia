...
def test_invalid_credentials():
    ...
    assert not any(connection_results.values()), "Connection should fail with invalid credentials"
...
def test_nonexistent_bucket():
    ...
    connection_results = client.test_connection()
    assert not all(connection_results.values()), "Connection should fail with nonexistent bucket"
...
def test_read_only_directory(buckia_client, temp_directory, test_file_factory, cleanup_remote_files):
...
def test_very_large_sync(buckia_client, test_directory_factory, remote_test_prefix, cleanup_remote_files):
...
def test_unicode_filenames(buckia_client, temp_directory, cleanup_remote_files):
    ...
    for filename, local_path in local_files.items():
        ...
        assert downloaded_content == original_content, f"Content mismatch for Unicode file: {filename}"
...
def test_network_throttling(buckia_client, test_file_factory, remote_test_prefix, cleanup_remote_files):
    ...
    try:
        ...
        assert download_path.stat().st_size == large_file.stat().st_size, "Downloaded file size mismatch"
